...
def test_unit():
    ...
    # -- U02: Exactly 12 tools (7 A1 + 5 A2) --
    ...
    record(
        "U02 Ahmad has exactly 12 tools",
        len(tools) == 12,
        f"Got {len(tools)} tools: {tool_names}",
    )

    # -- U03: All 12 expected tool names present --
    expected_tools = [
    ...
    # -- U05: handle_tool_call dispatches all 12 tools --
    import inspect
    ...
    # -- U12: Ahmad now has exactly 12 tools (redundant but explicit A2 check) --
    record(
        "U12 Ahmad has exactly 12 tools (A2 confirmation)",
        len(tools) == 12,
    ...
    # -- U15: handle_tool_call dispatches all 12 tools (explicit A2 check) --
    all_12 = expected_tools  # already includes A2 tools
    dispatch_missing_all = [t for t in all_12 if t not in source]
    record(
        "U15 handle_tool_call dispatches all 12 tools (A2 included)",
...
async def test_integration():
    ...
    async with async_session() as db:
        ...
        try:
            ...
            record(
                "I24 'switch to sarah' routes to ahmad (sarah absorbed into ahmad)",
                routed == "ahmad",
                f"Got: {routed}",
            )
        except Exception as e:
        ...
        try:
            ...
            record(
                "I25 'switch to norah' routes to ahmad (norah absorbed into ahmad)",
                routed == "ahmad",
                f"Got: {routed}",
            )
        except Exception as e:
