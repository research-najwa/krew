...
## Track 5: Unified Chat with @Mention Agent Routing

### Epic: UC -- Unified Chat Interface

Instead of separate conversations per agent (switching in sidebar), there is ONE chat thread where users @mention agents to route requests. Like Slack — one conversation, multiple participants.

**Example:**
```
User: @deema كم رصيد إجازاتي؟
Deema: رصيدك الحالي 14 يوم إجازة سنوية...

User: @ahmad show me the recruitment funnel
Ahmad: Here's the recruitment analytics for Q1...

User: @mohammad schedule an interview with candidate #245
Mohammad: Interview scheduled for Thursday 2:00 PM...
```

---

### Story UC-01: @Mention Parser

**ID:** S8-UC-01
**Title:** Parse @agent mentions from chat messages

> As the system, I need to detect @agent mentions in user messages and route to the correct agent, so that users can talk to any agent from a single chat thread.

**Acceptance Criteria:**

- [ ] A new utility function `parse_mention(message: str) -> tuple[str | None, str]` is created that:
  - Extracts the first `@agent_name` from the message
  - Returns `(agent_name, cleaned_message)` — the message with the @mention stripped
  - Case-insensitive: `@Deema`, `@deema`, `@DEEMA` all work
  - Supports English names: `@deema`, `@ahmad`, `@mohammad`, `@waleed`, `@yara`
  - Supports Arabic names: `@ديمة`, `@أحمد`, `@محمد`, `@وليد`, `@يارا`
  - If no @mention found, returns `(None, original_message)` — falls back to current routing logic (keyword-based orchestrator)
- [ ] The parser handles @mentions at any position: start, middle, or end of message
- [ ] The parser handles messages with text before and after the mention: "hey @deema what's my leave balance" → `("deema", "hey what's my leave balance")`
- [ ] Agent name aliases are supported: `@sarah` → `yara` (if renamed), future-proofed with a name map
...
### Story UC-02: Orchestrator @Mention Integration

**ID:** S8-UC-02
**Title:** Route messages via @mention before keyword fallback

> As the orchestrator, when a message contains an @mention, I should route directly to that agent without keyword matching, so that the user has explicit control over which agent handles their request.

**Acceptance Criteria:**

- [ ] The `AgentOrchestrator.route_message()` method is updated:
  1. **First:** Check for @mention via `parse_mention()`
  2. **If @mention found:** Route directly to that agent, skip keyword matching entirely
  3. **If no @mention:** Fall back to existing keyword-based routing (current behavior preserved)
- [ ] The cleaned message (without @mention) is passed to the agent so it doesn't see "@deema" in its input
- [ ] Invalid @mentions (e.g., `@unknown`) return a helpful message: "I don't recognize @unknown. Available agents: @deema, @ahmad, @mohammad, @waleed, @yara"
- [ ] The response includes which agent handled it (for UI attribution)
- [ ] Conversation context is maintained — if the user @mentions Deema, then sends a follow-up without @mention, the follow-up goes to Deema (last-mentioned agent stickiness)
...
### Story UC-03: Chat UI — Unified Thread with Agent Attribution

**ID:** S8-UC-03
**Title:** Single chat thread with @mention autocomplete and agent badges

> As a user, I want one chat thread where I can @mention different agents and see which agent responded, so that conversations feel natural like Slack.

**Acceptance Criteria:**

- [ ] The chat UI switches from "one agent at a time" to a **unified thread** mode:
  - All messages appear in one scrollable thread regardless of which agent responded
  - Each agent response shows the agent's **name and avatar** badge (e.g., "🔵 Deema" or "🟢 Ahmad")
  - Different agents get different accent colors for visual distinction
- [ ] **@mention autocomplete:**
  - When the user types `@` in the input field, a dropdown appears listing all available agents
  - Each entry shows: agent name (EN), agent name (AR), role description
  - Filtering as user types: `@de` → shows only Deema
  - Clicking or pressing Enter inserts the @mention into the input
  - The @mention is styled as a highlighted chip/tag in the input field
- [ ] **Agent badges on responses:**
  - Each agent response bubble has a small badge: avatar + name
  - Color coding: Deema=blue, Ahmad=green, Mohammad=purple, Waleed=orange, Yara=teal
- [ ] **Sidebar changes:**
  - The sidebar agent list changes from "click to switch conversation" to "click to @mention in current thread"
  - Or: sidebar becomes a reference panel showing agent capabilities, not a conversation switcher
  - Quick actions bar still updates per last-mentioned agent
- [ ] **Backward compatibility:**
  - If user sends a message without @mention, existing keyword routing still works
  - The thread shows which agent was auto-routed to (e.g., subtle label: "routed to Ahmad")

**Priority:** P0
**Dependencies:** S8-UC-01 (parser), S8-UC-02 (orchestrator)

---

### Story UC-04: Conversation Context per Agent

**ID:** S8-UC-04
**Title:** Maintain per-agent conversation context in unified thread

> As the system, when multiple agents participate in one thread, each agent should have its own conversation history context so that tool calls and responses don't bleed between agents.

**Acceptance Criteria:**

- [ ] The backend maintains **per-agent message history** within a single conversation:
  - When Deema is called, she sees only the messages addressed to her (+ her responses)
  - When Ahmad is called, he sees only his portion of the conversation
  - This prevents: user asks Deema about leave, then asks Ahmad for analytics — Ahmad doesn't get confused by Deema's leave context
- [ ] The unified thread in the DB stores all messages in one `conversation_id`, but each message has an `agent_name` field indicating which agent handled it
- [ ] The orchestrator builds the agent-specific context by filtering messages by `agent_name` before passing to the agent
- [ ] Shared context (employee info, tenant, date) is available to all agents as before
- [ ] The full unified thread is available for display in the UI (all messages, all agents)

**Priority:** P1
**Dependencies:** S8-UC-02 (orchestrator routing)

---

### Story UC-05: Agent Access Privileges per Employee

**ID:** S8-UC-05
**Title:** Control which agents each employee can interact with

> As an admin, I want to define which agents each employee (or role/department) can access, so that sensitive agents like Ahmad (CHRO analytics) are only available to authorized users.

**Design:**
Not every employee should see or @mention every agent. Access is controlled by role and department:

**Default Access Matrix:**

| Agent | Who can access | Rationale |
|-------|---------------|-----------|
| Deema | All employees | Employee services — everyone needs her |
| Waleed | All employees + managers get extra tools | Onboarding is universal, manager tools are role-gated |
| Mohammad | HR team + hiring managers | Recruitment is not for all employees |
| Ahmad | HR leadership + C-suite + department heads | CHRO analytics are executive-level |
| Yara | HR admins + IT admins | Agent factory is admin-only |
| Deployed agents | Per visibility settings (from Yara factory) | Already designed in yara_factory_enhancements.md |

**Acceptance Criteria:**

- [ ] A new configuration table `agent_access_rules` is created:

| Column | Type | Description |
|--------|------|-------------|
| `id` | UUID | PK |
| `tenant_id` | UUID | FK tenants |
| `agent_name` | String | e.g., "deema", "ahmad", "dept:{uuid}" |
| `access_type` | Enum | `all`, `role_based`, `department`, `specific_users` |
| `allowed_roles` | JSONB | e.g., ["hr_admin", "c_suite", "department_head"] |
| `allowed_departments` | JSONB | e.g., ["{dept_uuid}"] |
| `allowed_users` | JSONB | e.g., ["{emp_uuid}"] |
| `is_active` | Boolean | default True |

- [ ] The orchestrator checks access BEFORE routing:
  - If @mention targets an agent the employee can't access → "You don't have access to @ahmad. Contact your HR admin."
  - If keyword routing would send to a restricted agent → route to Deema as fallback with message: "I can help with general questions. For analytics, please contact your manager."
- [ ] The @mention autocomplete in the UI only shows agents the employee has access to
- [ ] The sidebar only shows accessible agents
- [ ] The quick actions bar only shows actions for accessible agents
- [ ] Default rules are seeded on tenant creation (Deema=all, Ahmad=hr+c_suite, etc.)
- [ ] Admins can modify access rules via API: `PUT /api/v1/agent-access/{agent_name}`
- [ ] For deployed agents (dept:{uuid}), access uses the `visibility` settings from `deployed_agents` table (already designed in Yara factory enhancements)

**Priority:** P0
**Dependencies:** S8-UC-02 (orchestrator), employee role data
...
### Track 5: Unified Chat with @Mention

| Capability | Story ID | Status |
|------------|----------|--------|
| @mention parser (EN + AR names) | S8-UC-01 | Covered |
| Orchestrator @mention routing | S8-UC-02 | Covered |
| Unified thread UI with agent badges | S8-UC-03 | Covered |
| @mention autocomplete dropdown | S8-UC-03 | Covered |
| Per-agent conversation context | S8-UC-04 | Covered |
| Agent access privileges per employee | S8-UC-05 | Covered |
...
| **P0** | S8-UC-01 (parser), S8-UC-02 (orchestrator), S8-UC-03 (UI), S8-UC-05 (access) | Unified Chat |
| **P1** | S8-UC-04 (per-agent context) | Unified Chat |
