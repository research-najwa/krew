...
async def get_suggestions(
    ...
    agent: str | None = Query(default=None, description="Active agent for priority boost"),
) -> SuggestionsResponse:
    ...
    # CUX-04-03: Agent-aware boost — suggestions matching active agent get priority bump
    if agent:
        agent_lower = agent.lower()
        for s in all_suggestions:
            if s.agent_target == agent_lower:
...
# ─── CUX-04: Agent-Scoped Quick Actions ─────────────────────────────────────
...
_ROLE_RESTRICTED_ACTIONS: dict[str, dict[str, set[str]]] = {
    "employee": {
...
async def get_quick_actions(
    ...
    """Get quick actions — agent-scoped when ?agent= provided, else role-based."""
    emp_id = chat_emp.employee_id
    tenant_id = chat_emp.tenant_id
    ...
    if agent and agent.lower() in _AGENT_ACTIONS:
        ...
        actions_list = [a for a in raw_actions if a["label_en"] not in restricted]
    else:
        ...
        actions_list = list(role_actions.get(role, _QUICK_ACTIONS_EMPLOYEE))
