...
async def chat_login(req: LoginRequest, request: Request, response: Response, db: AsyncSession = Depends(get_db)):
    ...
    # Additive: also set httpOnly cookie so the WebSocket gateway can authenticate.
    # Existing Bearer-header path keeps working — clients that ignore the cookie
    # are unaffected. CHAT_TOKEN_EXPIRE_HOURS = 12h.
    # secure=False in development so plain http://localhost can set the cookie.
...
async def chat_logout(
    ...
    """Logout — blacklist the chat token and clear the httpOnly cookie."""
    ...
    # Z6: Clear the httpOnly chat cookie so the browser discards it
    response.delete_cookie(key="krew_chat_jwt", path="/")
