...
# ---------------------------------------------------------------------------
# Role constants — single source of truth for persona-based scoping
# ---------------------------------------------------------------------------
...
# Convenience groups
ROLES_MANAGER_UP: set[str] = {
    ROLE_MANAGER, ROLE_DEPT_HEAD, ROLE_HR_SPECIALIST,
    ROLE_HR_MANAGER, ROLE_HR_ADMIN, ROLE_C_SUITE,
}
ROLES_DEPT_HEAD_UP: set[str] = {
    ROLE_DEPT_HEAD, ROLE_HR_MANAGER, ROLE_HR_ADMIN, ROLE_C_SUITE,
}
ROLES_HR_PLUS: set[str] = {
    ROLE_HR_SPECIALIST, ROLE_HR_MANAGER, ROLE_HR_ADMIN, ROLE_C_SUITE,
}
ROLES_HR_MANAGER_UP: set[str] = {
    ROLE_HR_MANAGER, ROLE_HR_ADMIN, ROLE_C_SUITE,
}
...
class ScopeContext:
    """Passed to tool implementations to scope data queries by persona."""
    ...
    def is_org_wide(self) -> bool:
        """Can this user see data across the entire org?"""
        return self.employee_role in (ROLE_HR_ADMIN, ROLE_HR_MANAGER, ROLE_C_SUITE)

    @property
    def is_department_scoped(self) -> bool:
        """Should results be limited to this user's department?"""
        return self.employee_role in (ROLE_DEPT_HEAD, ROLE_HIRING_MANAGER)

    @property
    def is_team_scoped(self) -> bool:
        """Should results be limited to direct reports?"""
        return self.employee_role == ROLE_MANAGER

    @property
    def is_self_only(self) -> bool:
        return self.employee_role in (ROLE_EMPLOYEE, ROLE_HR_SPECIALIST)


# Role labels for system prompt persona awareness
_ROLE_LABELS: dict[str, str] = {
    ROLE_EMPLOYEE: "a regular employee (self-service access only)",
    ROLE_MANAGER: "a manager with direct reports (can see team data)",
    ROLE_DEPT_HEAD: "a department head (can see full department data)",
    ROLE_HR_SPECIALIST: "an HR specialist",
    ROLE_HR_MANAGER: "an HR manager (org-wide HR access)",
    ROLE_HR_ADMIN: "an HR administrator (full HR access)",
    ROLE_IT_ADMIN: "an IT administrator",
    ROLE_HIRING_MANAGER: "a hiring manager (recruitment access for their department)",
    ROLE_C_SUITE: "a C-suite executive (full organizational access)",
}
...
class BaseAgent(ABC):
    ...
    # Subclasses define per-tool role visibility.
    # Keys = tool names, values = set of roles that can see/use the tool.
    # Tools NOT listed are visible to ALL roles that have agent-level access.
    ...
    def __init__(self, db: AsyncSession, tenant_id: UUID):
        ...
        self._employee_role: str = ROLE_EMPLOYEE
        self._employee_dept_id: UUID | None = None
        self._handoff_from: str | None = None
        self._is_first_message: bool = False
        self._last_suggestions: list[str] = []
        self._last_tool_calls: list[dict] = []
        self._scope: ScopeContext | None = None
    ...
    def get_visible_tools(self) -> list[dict]:
        """Return only tools this employee's role is allowed to see.

        Tools not listed in TOOL_VISIBILITY are visible to everyone.
        Tools listed in TOOL_VISIBILITY require the employee's role to be in the allowed set.
        This is what gets sent to Claude — proactive filtering, not reactive denial.
        """
        all_tools = self.get_tools()
        if not self.TOOL_VISIBILITY:
            return all_tools
        role = self._employee_role or ROLE_EMPLOYEE
        return [
            tool for tool in all_tools
            if tool["name"] not in self.TOOL_VISIBILITY
            or role in self.TOOL_VISIBILITY[tool["name"]]
        ]
...
{employee_name} is {_ROLE_LABELS.get(self._employee_role, "an employee")}. Only use the tools provided — they are already filtered for this user's access level. Do not suggest actions that require tools you cannot see.
{lang_instruction}
{cultural_context}
{greeting_hint}
...
- CRITICAL — ALWAYS USE TOOLS: Every request for data (leave balance, profile, documents, salary, team calendar, leave requests, policies, tickets, etc.) MUST be answered by calling the appropriate tool. NEVER answer from memory, conversation history, or the system prompt. Even if you already called the same tool earlier in this conversation, call it again — the UI renders each tool result as a fresh interactive card. Only skip tools for greetings, clarifications, and general conversation.
- If you cannot resolve something, explain why and offer to escalate to a human.
- For leave requests: When the employee provides dates and leave type, call preview_leave_request immediately to show an editable confirmation form card. If details are missing (no dates or type), ask briefly for the minimum info (leave type, start date, end date) — then call preview_leave_request. NEVER call submit_leave_request directly unless the employee's message starts with "CONFIRMED:". When you see "CONFIRMED:", call submit_leave_request immediately with the exact provided details.
- Saudi work week: Sunday–Thursday. Weekend: Friday–Saturday.
- Keep responses concise — this is a chat, not an email.
- CRITICAL: When you call tools, the UI renders the full results as rich interactive cards. The user can click entries in the card to drill into details. Therefore:
  • Do NOT write ANY text describing or summarizing the tool results — the card IS the answer.
  • After the tool call, ONLY output 1-3 suggested next actions as a numbered list. Nothing else.
  • Example output after a tool call:
    1. Filter by department
    2. Create a new job posting
    3. View candidate pipeline
- Suggested actions become clickable buttons in the UI. Keep each under 60 characters.
...
"""
    ...
    async def _resolve_dept_name(self, dept_id: UUID) -> str | None:
        """Look up department name by ID."""
    ...
    async def respond(
        ...
        # Inject department-scoped access boundary
        if self._employee_dept_id and self._employee_role not in ("hr_admin", "hr_manager"):
            dept_name = await self._resolve_dept_name(self._employee_dept_id)
            if dept_name:
                system_prompt += (
                    f"\n\nACCESS BOUNDARY: {employee_name}'s role is {self._employee_role} "
                    f"in the {dept_name} department. They can only view information about "
                    f"employees in their own department or their direct reports. "
                    f"Do NOT reveal employee data from other departments."
                )

        # Harden the system prompt with canary and security anchor
        system_prompt = build_hardened_system_prompt(system_prompt)

        # Build scope context for data scoping inside tool implementations
        self._scope = ScopeContext(
            employee_id=self._employee_id or "",
            employee_role=self._employee_role,
            department_id=self._employee_dept_id,
            tenant_id=self.tenant_id,
        )

        # Proactive tool filtering — only show tools this role is allowed to use
        tools = self.get_visible_tools()
            ...
            if response.stop_reason == "tool_use":
                ...
                for block in response.content:
                    if block.type == "tool_use":
                        # Defense-in-depth: block tool calls that slipped past filtering
                        if (
                            block.name in self.TOOL_VISIBILITY
                            and self._employee_role not in self.TOOL_VISIBILITY[block.name]
                        ):
                            logger.warning(
                                "RBAC blocked tool %s for role %s (employee %s)",
                                block.name, self._employee_role, employee_id,
                            )
                            result = json.dumps({
                                "error": True,
                                "message": "You do not have permission to use this tool.",
                                "message_ar": "ليس لديك صلاحية لاستخدام هذه الأداة.",
                            })
                            tool_results.append({
                                "type": "tool_result",
                        ...
                        try:
                            result = await asyncio.wait_for(
                                self.handle_tool_call(block.name, block.input),
                                timeout=120,
                            )
                        except asyncio.TimeoutError:
                            logger.error(
                                "Tool %s timed out after 120s for employee %s",
                                block.name, employee_id,
                            )
                            result = json.dumps({
                                "error": True,
                                "message": f"Tool '{block.name}' timed out. Please try again.",
                            })
                        ...
                        # Track tool call for suggestion generation + WS tool cards
                        ...
                        try:
                            ...
                            _is_error = bool(_parsed_result.get("error"))
