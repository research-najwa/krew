...
class SaudiLaborLawEngine:
    ...
    async def validate_leave_request(
        ...
        # Normalize leave_type to enum (defensive — callers should pass enum)
        if isinstance(leave_type, str):
            leave_type = LeaveType(leave_type)

        # Fetch tenant leave policy.
        # Note: leave_policies.leave_type is a varchar column (schema drift from the
        # native leavetype enum used on leave_requests/leave_balances), so we cast
        # the column to String to avoid "character varying = leavetype" mismatches.
