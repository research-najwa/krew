...
async def check_and_submit_leave(
        ...
        _policy_result = await db.execute(
            select(LeavePolicy).where(
                and_(
                    ...
                    cast(LeavePolicy.leave_type, String) == leave_type.value,
    ...
    # Fix 6: Account for days reserved by pending requests.
    # NOTE: Postgres forbids FOR UPDATE combined with aggregate functions, so this
    # aggregate read runs WITHOUT a row lock. Concurrency is still safe because the
    # LeaveBalance row above is already locked via with_for_update() — any concurrent
    # submission for the same (employee, leave_type, year) will block on that lock
