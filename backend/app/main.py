...
from app.api import agent_access
...
app.include_router(agent_access.router, prefix="/api/v1")
...
async def root():
    return {
        ...
        "version": "0.1.0",
        "agents": ["Deema", "Waleed", "Mohammad", "Yara", "Ahmad"],
        "status": "operational",
    }
...
@app.get("/home2")
async def home2_page():
    return FileResponse(STATIC_DIR / "home2.html")


@app.get("/chat-v2")
async def chat_v2_page():
    return FileResponse(STATIC_DIR / "chat-v2.html")
...
# Mount DM uploads directory for file/voice attachments
